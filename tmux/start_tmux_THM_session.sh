...
# Creating tmux session named "THM". add this script path to ~/.zshrc or ~/.bashrc.
...
if ! tmux has-session -t "$SESSION" 2>/dev/null; then
    ...
    tmux send-keys -t "$SESSION":0 'sudo openvpn $HOME/TryHackMe/vpn/academy-regular.ovpn' C-m
